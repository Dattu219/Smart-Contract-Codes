...
contract Lava {
  ...
  function submitRand(uint _value) public payable {
    ...
    // ¡Ì create Rand struct

    // ¡Ì add new Rand struct to rands

    // ¡Ì register/ledger deposit
  ...
  function submitPredWindow(uint[] _guess) public payable {
    ...
    // ¡Ì create accessible PredUnits

    // ¡Ì create accessible PredWindow
  ...
  function requestRand() public payable returns (uint) {
    ...
    // ¡Ì register/ledger payment

    // ¡Ì initiates auditing process (was there a correct prediction)

    // ¡Ì sends payments to appropriate players (rander recency or preder relative wager)

    // ¡Ì returns rand from timeline of most current timestamp
